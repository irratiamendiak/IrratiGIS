...
function classify(firms,context={}){
 ...
 const clusterEvidence=clusterFire||repeated>=2;
 ...
 if(repeated>=2&&!clusterFire){reasons.push("grupo espacial de detecciones")}
 ...
 if(parcelUrban) category=(industrialTag||veryNearIndustrial)?"probable_industrial_source":(strong||clusterEvidence?"possible_fire":"thermal_anomaly");
 ...
 else if((parcelRural||vegetation)&&(clusterEvidence||temporal>0)) category="probable_forest_fire";
 else if(ruralNatural&&(strong||clusterEvidence||temporal>0||repeated>0||score>=40)) category="probable_forest_fire";
 else if(ruralNatural) category="possible_fire";
 else if(vegetation&&(clusterEvidence||strong||temporal>0)&&score>=45) category="probable_forest_fire";
 else if(strong||clusterEvidence||score>=40) category="possible_fire";
...
if(window.IrratiGISFirms?.load&&!window.__irratiFirmsClassifierReload){window.__irratiFirmsClassifierReload=true;let n=0;const retry=()=>{if(!window.IrratiGISFirms?.load)return;try{window.IrratiGISFirms.load()}catch(e){console.warn("FIRMS reclasificación:",e)}if(++n<5)setTimeout(retry,400)};setTimeout(retry,300)}
})();
