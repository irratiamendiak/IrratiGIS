...
# Sensor/measure IDs copied from the working panel-fwi project supplied for
# comparison. This avoids making /stations/{id}/current a prerequisite for
# retrieving a completed day's readings (that endpoint was returning 403/timeout).
# Keep the API authentication flow unchanged; these are public sensor identifiers.
EUSKALMET_STATIC_MAPPING={
    'C023': {'temperature':('R0CY','measuresForAir','temperature'), 'humidity':('R0CY','measuresForAir','humidity'), 'wind_kmh':('Y024','measuresForWind','mean_speed'), 'rain_mm':('G4AS','measuresForWater','precipitation')},
    'C017': {'temperature':('R0D0','measuresForAir','temperature'), 'humidity':('R0D0','measuresForAir','humidity'), 'wind_kmh':('Y008','measuresForWind','mean_speed'), 'rain_mm':('G4DB','measuresForWater','precipitation')},
    'C058': {'temperature':('R0GB','measuresForAir','temperature'), 'humidity':('R0GB','measuresForAir','humidity'), 'wind_kmh':('Y0BE','measuresForWind','mean_speed'), 'rain_mm':('G460','measuresForWater','precipitation')},
    'C026': {'temperature':('R0IE','measuresForAir','temperature'), 'humidity':('R0IE','measuresForAir','humidity'), 'wind_kmh':('Y0DB','measuresForWind','mean_speed'), 'rain_mm':('G4B3','measuresForWater','precipitation')},
    'C028': {'temperature':('R0IC','measuresForAir','temperature'), 'humidity':('R0IC','measuresForAir','humidity'), 'wind_kmh':('Y0R8','measuresForWind','mean_speed'), 'rain_mm':('G4AF','measuresForWater','precipitation')},
}

async def station_payload_for_day(station,day):
    # Do not call /stations/{id}/current here: it can fail independently of
    # the dated readings endpoint. The fixed station names are sufficient metadata.
    return {'name':FIXED_STATIONS.get(station,station)}
...
async def api_get(path):
    # Diagnóstico temporal: endpoint, duración y estado HTTP; nunca registrar tokens.
    ...
    try:
        ...
        print(
            f"BASUGIX EUSKALMET HTTP endpoint={path} status={r.status_code} elapsed_s={elapsed:.2f}",
            flush=True,
        )
...
async def euskalmet_weather(station,day):
    """Fetch completed daily summaries using the known station sensor map."""
...
async def _v222_mapping(station,day):
    ...
    # Prefer the verified sensor map from panel-fwi. Cached BASUGIX mappings
    # are used only for any variable not present in that map.
    mapping={}
    for var,info in EUSKALMET_STATIC_MAPPING.get(station,{}).items():
        mapping[var]=tuple(str(x) for x in info)
    for var in TARGETS:
    ...
    if missing:
        # Retain the previous metadata discovery as a fallback for unknown stations.
        detail=await station_payload_for_day(station,day)
        discovered=await discover_daily_mapping(station,day,detail)
        mapping.update({k:v for k,v in discovered.items() if k not in mapping})
        missing=[v for v in TARGETS if v not in mapping]
    if missing:
        raise RuntimeError(f'No se pudo resolver sensor/medida para {station}: {missing}')
...
async def _v222_panel_read_hour(station, sensor, measure_type, measure_id, day, hour):
    """Read one hourly resource directly, following panel-fwi's API access pattern."""
    path=(f'/euskalmet/readings/forStation/{station}/{sensor}'
          f'/measures/{measure_type}/{measure_id}'
          f'/at/{day.year}/{day.month:02d}/{day.day:02d}/{hour:02d}')
    try:
        payload=await api_get(path)
    except RuntimeError as exc:
        # Euskalmet can return 404 for an hour without observations; treat it as empty.
        if 'HTTP 404' in str(exc):
            return []
        raise
    return _v2243_slot_points(payload)


async def _v222_panel_value_at_hour(station, mapping, var, day, target_hour):
    sensor, measure_type, measure_id=mapping[var]
    points=await _v222_panel_read_hour(
        station,sensor,measure_type,measure_id,day,target_hour
    )
    value,tm,path=_v222_nearest(points,target_hour,V22_NOON_TOLERANCE_MIN)
    if value is None:
        raise RuntimeError(
            f'panel-fwi: sin observación de {var} para {station} '
            f'{day.isoformat()} alrededor de {target_hour:02d}:00; '
            f'endpoint horario, puntos={len(points)}'
        )
    return value,tm,{'hour_points':len(points),'selected_path':path}


async def _v222_panel_rain24(station,mapping_today,mapping_prev,day,target_hour):
    """Accumulate the individual 10-minute rain observations as panel-fwi does."""
    prev_day=day-timedelta(days=1)
    total=0.0
    n=0
    missing=[]
    # Previous day: target hour through 23; current day: 00 through target hour.
    branches=[(prev_day,mapping_prev['rain_mm'],range(target_hour,24)),
              (day,mapping_today['rain_mm'],range(0,target_hour+1))]
    for src_day,(sensor,measure_type,measure_id),hours in branches:
        for hour in hours:
            pts=await _v222_panel_read_hour(
                station,sensor,measure_type,measure_id,src_day,hour
            )
            if not pts:
                missing.append(f'{src_day.isoformat()} {hour:02d}:00')
            for hhmm,val,_ in pts:
                try:
                    hh,mm=map(int,hhmm.split(':'))
                except Exception:
                    continue
                mins=hh*60+mm
                cutoff=target_hour*60
                use=((src_day==prev_day and mins>cutoff) or
                     (src_day==day and mins<=cutoff))
                if use and 0.0<=float(val)<=100.0:
                    total+=float(val)
                    n+=1
    if n==0:
        raise RuntimeError(f'panel-fwi: no hay lecturas de lluvia para {station} {day}')
    # Same quality gate as panel-fwi (at least ~90% of 144 ten-minute slots).
    if n<130:
        raise RuntimeError(
            f'panel-fwi: lluvia incompleta {station} {day}: {n}/144 tramos; '
            f'horas vacías={missing[:8]}'
        )
    return round(total,2),n,{'missing_hours':missing,'coverage_pct':round(100*n/144,1)}


async def _v222_recent_api_weather(station,day):
    """Use panel-fwi's direct hour-by-hour endpoints and 10-minute rain accumulation."""
    mapping_today=await _v222_mapping(station,day)
    mapping_prev=await _v222_mapping(station,day-timedelta(days=1))
    values={}
    meta={}
    target_hour=V22_NOON_HOUR

    # Match panel-fwi: use the measurement at the configured observation hour.
    for var in ('temperature','humidity','wind_kmh'):
        val,tm,info=await _v222_panel_value_at_hour(
            station,mapping_today,var,day,target_hour
        )
        values[var]=val
        meta[var]={'time':tm,**info}

    wind_kmh=max(0.0,float(values['wind_kmh'])*3.6)
    rain24,rain_n,rain_details=await _v222_panel_rain24(
        station,mapping_today,mapping_prev,day,target_hour
    )
    _,direction=await web_summary_wind(station,day)

    return {
        'source':'Euskalmet API direct hourly readings (panel-fwi method)',
...
async def debug_v221052_find_first_substitutions_fast(
    ...
    async def live_check(sid,d):
        try:
            ...
            print(f"BASUGIX LIVE CHECK TIMEOUT station={sid} day={d.isoformat()} limit_s={per_check_timeout}", flush=True)
            return {
...
async function loadForDate(dayValue=null){
 ...
 try{
   ...
   if(!dayValue && data.needs_live_refresh){
     status.textContent='✓ Último día completo mostrado · intentando recuperar la fecha operativa…';
...
async function refreshOperationalD0InBackground(){
  // Se ejecuta cuando falta la fecha operativa solicitada, tanto si es D0 como D-1.
  ...
  try{
    ...
    status.textContent='✓ BASUGIX actualizado · fecha operativa recuperada desde Euskalmet';
  }catch(e){
    // Timeout/corte: conservamos el último día SQLite ya mostrado e informamos del fallo.
    status.textContent='⚠ Se mantiene el último día completo; Euskalmet no ha respondido para la fecha solicitada.';
    console.warn('Actualización operativa en segundo plano no disponible',e);
  }finally{clearTimeout(timer);}
}
