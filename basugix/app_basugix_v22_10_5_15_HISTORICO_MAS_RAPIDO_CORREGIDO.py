...
async def _v221051_backup_candidates(target_station,force_catalog=False):
    ...
    for st in catalog:
        sid=st['station_id']
        # BASUGIX: el respaldo se limita estrictamente a las cinco estaciones acordadas.
        if sid not in FIXED_STATION_IDS or sid==target_station: continue
...
async def api_v221052_latest_fast():
    ...
    # BASUGIX data-integrity rule: never return an older SQLite day as if it
    # represented the requested operational day. Return five explicit empty
    # station records instead; the frontend can keep the date and show 'Sin dato'.
    return {
        'ok':True,
        'version':'BASUGIX · fecha solicitada sin observaciones confirmadas',
        'writes_to_database':False,
        'selected_day':desired.isoformat(),
        'requested_day':desired.isoformat(),
        'live_fallback':False,
        'needs_live_refresh':False,
        'mode':'requested_day_unavailable_no_stale_fallback',
        'error':'No hay un paquete de observaciones confirmado para la fecha solicitada; no se sustituye por histórico antiguo.',
        'stations':[{
            'station_id':sid,
            'station_name':FIXED_STATIONS[sid],
            'lat':STATION_COORDS[sid]['lat'],
            'lon':STATION_COORDS[sid]['lon'],
            'day':desired.isoformat(),
            'ok':False,
            'temperature':None,'humidity':None,'wind_kmh':None,'rain_mm':None,
            'wind_direction_deg':None,'wind_direction_cardinal':None,
            'fwi':None,'isi':None,'bui':None,'dc':None,'dmc':None,'ffmc':None,
            'ire_gip':None,'fwi_level':'Sin dato','ire_gip_level':'Sin dato',
            'data_quality':'Sin dato',
            'error':'No hay observaciones confirmadas para esta fecha.',
            'station_fallback_used':False,
        } for sid in FIXED_STATION_IDS],
    }
...
async def api_v22103_live(day:str|None=None, force:int=0, _skip_prev_sync:bool=False):
    ...
    if not any(bool(x.get('ok')) for x in out) and day is None:
        fallback_payload=_v221052_latest_complete_db_payload(d)
        if fallback_payload is not None:
            fallback_payload=dict(fallback_payload)
            fallback_payload['requested_day']=d.isoformat()
...
async function refreshOperationalD0InBackground(){
  ...
  try{
    ...
    if(!fresh.ok || fresh.selected_day!==desired || !Array.isArray(fresh.stations)) return;
    data=fresh;
    cards();
    const firstValid=data.stations.find(s=>s.ok!==false)||data.stations[0];
    ...
    status.textContent=fresh.stations.some(s=>s.ok!==false)
      ? '✓ BASUGIX actualizado · observaciones válidas para la fecha'
      : '⚠ Sin observaciones válidas para '+desired+' · no se muestran datos históricos como actuales';
  }catch(e){
...
// Al abrir la página, consultar explícitamente el día operativo; no cargar SQLite antiguo.
loadForDate(initialToday);
</script>
</body>
</html>"""
