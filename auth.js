(() => {
  ...
  function showLogin(message = "Acceso protegido") {
    let old = document.getElementById("irratiLoginBackdrop");
    if (old) old.remove();
    const back = document.createElement("div");
    back.id = "irratiLoginBackdrop";
    ...
    box.style.cssText = "width:min(420px,94vw);background:#fff;border-radius:16px;padding:18px;box-shadow:0 12px 40px rgba(0,0,0,.3);font:14px system-ui;color:#16231c";
    ...
    const user = box.querySelector("#irratiUser");
    const pass = box.querySelector("#irratiPass");
    const remember = box.querySelector("#irratiRemember");
    const login = box.querySelector("#irratiLogin");
    const msg = box.querySelector("#irratiLoginMsg");
  ...
  async function validateToken(token) {
    const response = await apiFetch("/api/me", {headers:{"Authorization":`Bearer ${token}`}});
