(() => {
  ...
  function installFirmsLeafletOverlay(){
    ...
    if(!L.Control.Layers.prototype.__irratiFirmsPatched){
      ...
      L.Control.Layers.prototype._update=function(){
        if(this._layers&&!this._layers.some(x=>x.layer===firmsLayer)){
          this._layers.push({layer:firmsLayer,name:"🚨 NASA FIRMS",overlay:true});
        }
    ...
    const pulse=L.layerGroup().addTo(leafletMap);
    leafletMap.removeLayer(pulse);
    return true;
  }
  ...
  function setupAppArchitecture(){
    ...
    if(originalMapHead){
      ...
      if(badges)badges.innerHTML=`<span class="badge">🔥 Erreketa baimenduak</span> <span class="badge blue">🚨 NASA FIRMS</span>`;
    }
